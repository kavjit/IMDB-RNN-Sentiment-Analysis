...
# -*- coding: utf-8 -*-
"""
Created on Wed Oct 31 11:35:53 2018
...
class StatefulLSTM(nn.Module):
    def __init__(self,in_size,out_size):
        ...
        self.lstm = nn.LSTMCell(in_size,out_size)   #the lstm block is avail in pytorch itself
    ...
    def forward(self,x):
        ...
        if self.h is None:  #the starting values are None
            state_size = [batch_size, self.out_size]    #used to define the array size for the zero thing next step
...
class RNN_model(nn.Module):
    ...
    def forward(self, x, t, train=True):    #x is size batchsize x sequence lenght - all inputs are fixed length

        embed = self.embedding(x) # batch_size, time_steps, features

        no_of_timesteps = embed.shape[1]    #this is length of sequence - how many times the words are passed through network
        ...
        for i in range(no_of_timesteps):
            ...
            outputs.append(h)   #each append inputs the h value for a word to a list
                                #the lstm automatically stores and updates c and h

        outputs = torch.stack(outputs) # (time_steps,batch_size,features)
        outputs = outputs.permute(1,2,0) # (batch_size,features,time_steps)

        pool = nn.MaxPool1d(no_of_timesteps)    #this is similar to the average in BOW - it outputs 1 resultant vector for each batch
        h = pool(outputs)
        h = h.view(h.size(0),-1)
        #h = self.dropout(h)
