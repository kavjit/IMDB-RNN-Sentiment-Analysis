...
# -*- coding: utf-8 -*-
"""
Created on Tue Oct 30 10:10:13 2018
...
#this list has all the filenames for all the reviews
filenames = pos_filenames + neg_filenames + unsup_filenames

#each file contains only one line, read the line and replace values, store each review in a single list
#tokenize converts a line or review to a list of all the words and punctuation seperatley, a list of lists for
#all reviews is then created - x_train
#tokenize also splits by punctuation it's becomes it ' s
count = 0
x_train = []
...
all_tokens = itertools.chain.from_iterable(x_train)     #extracts all the induvidual values from the nested list
...
#Organizing the word token by frequency in dataset
## let's sort the indices by word frequency instead of random
x_train_token_ids = [[word_to_id[token] for token in x] for x in x_train] #id for all tokens off all data -still nested lists
count = np.zeros(id_to_word.shape)  #to count occurence of each unique token
for x in x_train_token_ids:
    for token in x:
        count[token] += 1   #since index corresponds to token id
indices = np.argsort(-count)    #get id values of words in sorted order of frequency
...
#we can also use only the firsr 8000 most freq words and put the others in 0
...
count = 0
for line in lines:
    line = line.strip() #glove contains rows for each word and all the features
    line = line.split(' ')
...
x_train_token_ids = [[word_to_id.get(token,-1)+1 for token in x] for x in x_train]  #mapping x_train tokens wrt to glove
x_test_token_ids = [[word_to_id.get(token,-1)+1 for token in x] for x in x_test]
